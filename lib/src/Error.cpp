...
Error::Error(std::string const& message, std::map<std::string, std::shared_ptr<boom::Shared>> const& data)
    : _message(message)
    , _data(data) {}
...
std::shared_ptr<boom::Shared> Error::_getData(std::string const& key) const {
...
void Error::setData(std::string const& key, std::shared_ptr<boom::Shared> value) {
    _data[key] = value;
}
