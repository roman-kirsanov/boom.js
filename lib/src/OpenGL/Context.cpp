    ...
    , _impl(nullptr)
{
    _implInit(shared);
    _implMakeCurrent();
    _basicShaders = boom::MakeShared<boom::Shaders>(BASIC_VERTEX_SHADER, BASIC_FRAGMENT_SHADER);
    _imageShaders = boom::MakeShared<boom::Shaders>(TEXTURE_VERTEX_SHADER, TEXTURE_FRAGMENT_SHADER);
}
...
std::shared_ptr<boom::Context const> Context::Shared() {
    static auto context = std::shared_ptr<boom::Context>(new boom::Context(true));
    return context;
}
