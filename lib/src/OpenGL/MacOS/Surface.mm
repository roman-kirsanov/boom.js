...
void Surface::_implInit() {
    ...
    if (_type == boom::SurfaceType::Window) {
        ...
        _impl->impl->view = [[NSOpenGLView alloc]
            initWithFrame: _impl->impl->window.contentView.frame
            pixelFormat: [_impl->context->_impl->context pixelFormat]// [NSOpenGLView defaultPixelFormat]
        ];
        [_impl->impl->view setOpenGLContext: _impl->context->_impl->context];
        [_impl->impl->window setContentView: _impl->impl->view];
        ...
        if (status1 != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "ERROR: boom::Surface::_implInit() failed: Failed to make complete intermediate framebuffer object" << std::endl;
            ::exit(-1);
        }
        ...
        if (status2 != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "ERROR: boom::Surface::_implInit() failed: Failed to make complete texture framebuffer object" << std::endl;
            ::exit(-1);
        }
